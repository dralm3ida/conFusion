...
.controller('IndexController', ['$scope', 'dish', 'promotion', 'leader', 'menuFactory', 'promotionFactory', 'corporateFactory', 'baseURL', 
function($scope, dish, promotion, leader, menuFactory, promotionFactory, corporateFactory, baseURL) {
...
.controller('AboutController', ['$scope', 'leaders', 'corporateFactory', 'baseURL', function($scope, leaders, corporateFactory, baseURL) {
  ...
  console.log("leaders",$scope.leaders);
}])
      ...
      favorites = $localStorage.getObject('favorites','{}');
        ...
        for (var i = 0; i < favorites.length; i++) {
            for (var j = 0; j < dishes.length; j++) {
                if (dishes[j].id*1 === favorites[i].id*1)
                    out.push(dishes[j]);
            }
        }
        return out;
